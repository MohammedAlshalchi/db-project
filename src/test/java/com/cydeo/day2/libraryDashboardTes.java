...
public class libraryDashboardTes {
...
@Test
public void testDashboardUsers (){
     int actualUsersNumber = 141;
    ...
    int expectedResult =  Integer.parseInt( DB_Util.getFirstRowFirstColumn() )  ;
      ...
      assertEquals(actualUsersNumber,expectedResult);
    ...
    @Test
    public void testBorrowedBooksUI_DB() {
        ...
        DB_Util.createConnection(url,username,password);
        DB_Util.runQuery("SELECT COUNT(*) AS USER_COUNT FROM book_borrow");


        int actualBorrowedBookCount = 11;
        int expectedBorrowedBookCountFromDB = Integer.parseInt( DB_Util.getFirstRowFirstColumn() )  ;

        assertEquals(actualBorrowedBookCount, expectedBorrowedBookCountFromDB);
    }
}
