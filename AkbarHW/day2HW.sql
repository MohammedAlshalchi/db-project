...
FROM EMPLOYEES
WHERE EMPLOYEE_ID <> 110
AND EMPLOYEE_ID BETWEEN 100 AND  115 ;
...
SELECT FIRST_NAME,SALARY,(SALARY * EMPLOYEES.COMMISSION_PCT) AS "COMMISSION_VALUE"
FROM EMPLOYEES
...
SELECT *
FROM JOBS
WHERE MIN_SALARY > 1000 AND  MAX_SALARY < 30000;
...
--Employees Table
    ...
    --3.1  Display Employees FIRST_NAME , LAST_NAME , FULL_NAME, LENGTH OF FULL_NAME
...
SELECT Sum(SALARY)
from EMPLOYEES
where initcap(JOB_ID) like '%Clerk%';
...
From COUNTRIES group by REGION_ID having count(REGION_ID)>5;
